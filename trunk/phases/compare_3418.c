...
//			printf("i %d j %d k %d: %s\n", i, j, k, lineinput);
	...
	for( ;; )
	{
		...
		if( DB_TRIG_VAR( &trig_info) == DB_ATSC2_VAR )
                {
                        /***************************************************/
                        /* Read the database for the ATSC data from AB3418 */
                        /***************************************************/
		...
		for( i=1; i <= no_phases; i++ )
		{
			...
			else if( trig_type == ATSC_SOURCE_AB3418 )
			{
				...
				else if( green2[phase_num] == 0 )
				{
					if( (green2[phase_num] != old_green2[phase_num]) && (old_green2[phase_num] != -1) )
						start_yellow2[phase_num] = time_sec;
					if( (old_green2[phase_num] == -1) || (start_yellow2[phase_num] == -1) )
					{
						signal_color2[phase_num] = SIGNAL_STATE_UNKNOWN;
						start_yellow2[phase_num] = -1;
					}
					else if( (time_sec - start_yellow2[phase_num]) <= yellow_length[tindex][i] )
                                        {
					...
					&& (signal_color2[phase_no[i+1]] != old_color2[phase_no[i+1]]))
				{
					...
					if( phase_no[i+1] == 2 )
					{
						ab3418_trig[0][t2_p2] = trig2_time.tv_sec;
                                		ab3418_trig[1][t2_p2] = trig2_time.tv_nsec/1000000;
						t2_p2++;
					}
					else
					{
						ab3418_trig[2][t2_p4] = trig2_time.tv_sec;
                                		ab3418_trig[3][t2_p4] = trig2_time.tv_nsec/1000000;
						t2_p4++;
					}
				}
                        }
		...
		if( (time_sec >= 600.0) && (print_flag==FALSE) )
		{
        		for( i=0; i < min(t1_p4,t2_p4); i++ )
			{
				...
				printf("%d.", ab3418_trig[0][i]);	/* AB3418 trigger in sec */
				printf("%03d ", ab3418_trig[1][i]);	/* AB3418 trigger in msec */

				/* For phase 4 */
				printf("%d.", sniffer_trig[2][i]);	/* Sniffer trigger in sec */
				printf("%03d ", sniffer_trig[3][i]);	/* Sniffer trigger in msec */
				printf("%d.", ab3418_trig[2][i]);	/* AB3418 trigger in sec */
				printf("%03d\n", ab3418_trig[3][i]);	/* AB3418 trigger in msec */
			}
			print_flag = TRUE;
		}
	}
