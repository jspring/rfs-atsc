...
int main( int argc, char *argv[] )
{
	...
	db_data_typ	db_data;
	unsigned trig_var_list[SMSARRMAX],trig_typ_list[SMSARRMAX]; 
	...
	trignums = SMSARRMAX;	
	for (i=0;i<SMSARRMAX;i++)
	{
		trig_var_list[i] = DB_SMS_OBJARR0_VAR+i;
		trig_typ_list[0] = DB_SMS_OBJARR0_TYPE+i;	
	}	
	// creat UDP socket 
	...
	if( (pclt = database_init(argv[0], hostname, DEFAULT_SERVICE,
			COMM_PSX_XPORT, &trig_var_list[0], &trig_typ_list[0], trignums )) == NULL )
	{
		fprintf(stderr,"Database initialization error in %s\n",argv[0]);
		tsp_done( pclt, &trig_var_list[0], &trig_typ_list[0], trignums );
		exit( EXIT_FAILURE );
	}
	// exit 
	...
	for( ;; )
	{
		...
		psmsarr = (smsobjarr_t *) db_data.value.user;						
		if (scn_prn == 1)
		{
		...
		lenSendMsg = assemSMS(buffer,psmsarr);
