...
int main(int argc, char *argv[])
{
	...
	strncpy(ser_name, "/dev/ttyS0", 80);

        while ((ch = getopt(argc, argv, "i:s:t:k:d:x:")) != EOF)
	{
                switch (ch)
		{
		case 'i': initiator = atoi(optarg);
			  break;
                ...
                case 't': time_out = atoi(optarg);
                          break;
                case 'k': keep_sending = atoi(optarg);
                          break;
		case 'd': domain = strdup(optarg);
			  break;
                case 'x': xport = atoi(optarg);
                          break;
...
//		tmg.exec_num++;
		...
		fflush(stdout);
#endif
		tv.tv_sec = time_out / 1000;/*necessary to reset tv struct in Linux*/
		tv.tv_usec = (time_out % 1000) *1000;
